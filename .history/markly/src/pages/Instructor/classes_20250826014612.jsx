...
export default function Classes() {
    ...
    useEffect(() => {
        const fetchClasses = async () => {
            ...
            const res = await axios.get("http://localhost:5000/api/auth/classes", {
  ...
  fetchClasses();
    ...
    const handleSave = async (e) => {
        ...
        try {
            if (isEditing) {
            await axios.put(`http://localhost:5000/api/auth/classes/${editId}`, payload, {
                headers: { Authorization: `Bearer ${token}` }
            });
            alert("Class updated successfully!");
            } else {
            await axios.post("http://localhost:5000/api/auth/classes", payload, {
                headers: { Authorization: `Bearer ${token}` }
            });
            ...
            const updated = await axios.get("http://localhost:5000/api/auth/classes", {
            headers: { Authorization: `Bearer ${token}` }
            });
        ...
        const handleDelete = async (id) => {
            ...
            try {
                const token = localStorage.getItem("token") || sessionStorage.getItem("token");
                await axios.delete(`http://localhost:5000/api/auth/classes/${id}`, {
                headers: { Authorization: `Bearer ${token}` }
                });
