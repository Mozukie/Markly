...
function MyQRCode() {
  return <QRCode value="https://your-url.com" />;
}
            ...
            <main className="container mx-auto p-4">
                ...
                {qrModal && selectedClass && (
                    <div className="fixed inset-0 bg-opacity-50 flex items-center justify-center z-50">
                        ...
                        <QRCode
                            value={`http://localhost:5000/attendance/${selectedClass._id}?expires=${Date.now() + parseInt(expiryDuration) * 1000}`}
                            size={200}
                        />
        ...
        {classList.length === 0 ? (
            ...
            <ul className="space-y-3">
                {classList.map((cls) => (
                    ...
                    <div>
                        <label className="block text-gray-700 mr-2 mt-2">Time Limit</label>
                        <select value={classData.expiry} onChange={(e) => setClassData({ ...classData, expiry: e.target.value })} className="mt-2 border px-2 py-1 rounded"a>
